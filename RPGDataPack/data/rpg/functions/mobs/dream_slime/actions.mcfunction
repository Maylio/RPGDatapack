#行動パターン
    ...
    #150
        #飛び掛かり攻撃
        #プレイヤーの方を向かせる
        execute as @e[tag=dream_slime,scores={AI_tick=150},nbt={NoAI:true}] at @s facing entity @p eyes run tp @s ~ ~ ~ ~ 0
...
#合体
    ...
    execute as @e[tag=dream_slime,tag=!dream_slime_union,tag=!union_slime] store result score @s slime_get_health run data get entity @s Health
    execute as @e[tag=dream_slime,tag=!dream_slime_union,tag=!union_slime,scores={slime_get_health=..5}] run tag @s add dream_slime_union
